...
ObjectFunction = function (x, vectorized = FALSE, fun = branin) {
  l.lim.uc <- c(-5.1, -0.1)
  u.lim.uc <- c(0, 5)
  L <- 2
  val.uc <- 20
  ...
  if (vectorized == TRUE) {
    ...
    for (i in 1:nrow(x)) {
      ...
      if (all(x.row > matrix(l.lim.uc, 1, L) &
              x.row < matrix(u.lim.uc, 1, L))) {
        val[i,] <- as.matrix(val.uc)
...
branin <- function(xx, a = 1, b = 5.1 / (4 * pi ^ 2),
                   c = 5 / pi, r = 6, s = 10, t = 1 / (8 * pi)) {
  ##########################################################################
  #
  # BRANIN FUNCTION
  #
